...
JNIEXPORT void JNICALL Java_OperatingSystem_setWindowsHook(JNIEnv *env, jobject obj,
														   jint jIdHook, jstring jClassName, jstring jWindowName) {
	
    const char* className=env->GetStringUTFChars(jClassName,0);
    const char* windowName=env->GetStringUTFChars(jWindowName,0);
	int idHook = (int)jIdHook;
	parser = env->NewGlobalRef(obj);
	env->GetJavaVM(&javaVM);

	DWORD tid;
	HWND hwndBaseWindow = FindWindow(className, windowName); //FindWindow("SunAwtFrame", "MDS");
	...
	if(idHook == OperatingSystem_WH_GETMESSAGE) {
		if(hhookMsgHook == NULL) {
			hhookMsgHook = SetWindowsHookEx(WH_GETMESSAGE, GetMsgProc, NULL, tid);
		} else {
			ThrowLastException(env, "Specified hook already created.");
		}
	} else {
		ThrowLastException(env, "Specified hook type does not support MDS.");
	}
...
JNIEXPORT void JNICALL Java_OperatingSystem_unHookWindowsHook(JNIEnv *, jobject, jint jIdHook) {
	if(hhookMsgHook != NULL)
...
JNIEXPORT void JNICALL Java_OperatingSystem_printConsole__Ljava_lang_String_2(JNIEnv *env, jobject,  jstring text) {
	const char* txt=env->GetStringUTFChars(text,0);
	PrintW(txt);
}


JNIEXPORT void JNICALL Java_OperatingSystem_printConsole__Ljava_lang_String_2I(JNIEnv *env, jobject,  jstring text, jint color) {
	const char* txt=env->GetStringUTFChars(text,0);
	PrintW(txt, color);
}


JNIEXPORT void JNICALL Java_OperatingSystem_setConsoleTitle (JNIEnv *env, jobject,  jstring text) {
	const char* txt=env->GetStringUTFChars(text,0);
	SetConsoleTitle(txt);
}

JNIEXPORT void JNICALL Java_OperatingSystem_ejectRemovableMedia (JNIEnv *env, jobject,  jstring drive) {
	const char* drv=env->GetStringUTFChars(drive,0);
	if(!EjectVolume(TEXT(drv)[0]))
		ThrowLastException(env, "Failure ejecting drive.");
}

JNIEXPORT void JNICALL Java_OperatingSystem_setConsoleForegroundColor (JNIEnv *, jobject, jint color) {
